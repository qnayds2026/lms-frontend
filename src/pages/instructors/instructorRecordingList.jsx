...
function CourseCardSkeleton() {
  return <div className="bg-white rounded-2xl border border-slate-200 h-40 animate-pulse" />;
}

const InstructorRecordingsList = () => {
  ...
  return (
    <div className="p-6 md:p-10 bg-slate-50 min-h-screen">
      ...
      <span
        ...
        style={mono}
      >
        <Terminal className="w-3.5 h-3.5" /> recordings
      </span>
      <h1 className="mt-3 text-3xl font-semibold text-slate-900" style={display}>
        Recordings
      </h1>
      <p className="text-slate-500 mt-1.5 text-sm">
        Pick a course to manage its lesson recordings.
      </p>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-8">
        ...
        {!loading &&
          courses.map((course) => (
            <Link
              key={course.id}
              to={`/instructor/courses/${course.id}/recordings`}
            ...
            >
              ...
              <div className="mt-1.5 flex items-center gap-3 text-xs text-slate-400" style={mono}>
              ...
              <span className="mt-auto pt-4 inline-flex items-center gap-1 text-sm font-medium text-sky-600 group-hover:gap-2 transition-all">
                Manage Recordings <ArrowRight className="w-3.5 h-3.5" />
              </span>
            </Link>
          ))}
      </div>
...
export default InstructorRecordingsList;