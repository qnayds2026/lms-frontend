...
function EditRecordingModal({ recording, onClose, onUpdated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err?.response?.data?.error || "Failed to update recording.");
    } finally {
      setSaving(false);
    }
  };
...
const AdminManageRecordings = () => {
  ...
  const handleUpdated = (moduleId, updated) => {
    setModules((prev) =>
      prev.map((m) =>
        m.id === moduleId
          ? {
  ...
  const handleDelete = async (recording, moduleId) => {
    if (!window.confirm(`Delete "${recording.title}"? This can't be undone.`)) {
      return;
    }
...
export default AdminManageRecordings;